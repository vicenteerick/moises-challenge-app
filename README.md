...
5. **CarPlay**: the app declares a `CPTemplateApplicationScene` in `Info.plist` with `CarPlaySceneDelegate`. To run CarPlay, use Xcode’s CarPlay simulator (choose `I/O > External Displays > CarPlay`); playback uses **audio** background mode.
